...
namespace SiemensProject
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //}
            string jsontext_e = File.ReadAllText(@"event.jason");
            bool success2 = false;

            try
            {
                var tmpObj = JsonValue_e.Parse(jsontext);
                JsonConvert.PopulateObject(jsontext, allevents); //Daca Jsonul este empty da eroare, insa este necesar pentru a trece datele in array din json
                success2 = true;
            }
            catch (FormatException fex)
            {
